...
with col2:
    st.selectbox("Model Selection",options=["Random Forest Classifier","Gradient Boost"],key="model_select")

# initializa select model session state
if "model_select" not in st.session_state:
    st.session_state["model_select"] = "Random Forest Classifier"
...
def make_prediction():
    ...
    if st.session_state["model_select"] == "Logistic Regression":
        ...
        if random_forest_response.status_code == 200:
            ...
            st.error(f"Error: {  random_forest_response.json()["error"]}")
    else:
